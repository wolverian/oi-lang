...
(defn string->runtime* [s] (-> s oi/parse oi/ast->runtime*))

(deftest simple-ast->runtime*-tests
  (is (oi/oi-= (string->runtime* "42") [(oi/oi-number 42)]))
  (is (oi/oi-= (string->runtime* "42 < 22") [oi/oi-false]))
  (is (oi/oi-= (string->runtime* "2<1") [oi/oi-false]))
  (is (oi/oi-= (string->runtime* "1<2") [oi/oi-true]))
  (is (oi/oi-= (string->runtime* "42 > 22") [oi/oi-true])))

(deftest list-ast->runtime*-tests
  (is (oi/oi-= (string->runtime* "list(1, 2, 3)")
               [(oi/oi-list (oi/oi-number 1) (oi/oi-number 2) (oi/oi-number 3))]))
  (is (oi/oi-= (string->runtime* "x := list(1, 2, 3); x")
               [(oi/oi-list (oi/oi-number 1) (oi/oi-number 2) (oi/oi-number 3))])))

(defn string->result* [s] (-> s string->runtime* oi/eval*))
